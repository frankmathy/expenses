...
class FoursquareClient {
    ...
    func search(atLocation currentLocation : CLLocationCoordinate2D, radius: Int, resultLimit: Int, completionHandler: @escaping ([FoursquareVenue]?, Error?) -> Swift.Void) {
        let today = Date()
        let url = "https://api.foursquare.com/v2/search/recommendations?ll=\(currentLocation.latitude),\(currentLocation.longitude)&v=\(today.asYYYYMMDDString)&intent=browse&limit=\(resultLimit)&radius=\(radius)&client_id=\(AppCredentials.foursquareClientId)&client_secret=\(AppCredentials.foursquareClientSecret)"
        print(url)
